...
BEGIN
 ...
 INSERT INTO public.dp_colaboradores(company_id,nome,unidade_id,cargo_id,regime,forma_pagamento,valor_diaria) VALUES(c,'L02 FLUXO FICTICIO',unit,cargo,'freelancer','diarista',200) RETURNING id INTO emp;
...
DO $offer$ DECLARE v record; BEGIN
 ...
 IF v.regime_snapshot::text IS DISTINCT FROM 'freelancer' OR v.compatibilidade IS DISTINCT FROM 'integral' OR v.status::text <> 'pendente' OR (v.remuneracao_snapshot->>'valor_previsto')::numeric IS DISTINCT FROM 200 THEN RAISE EXCEPTION 'offer snapshot incorrect'; END IF;
...
END $partial$;
RESET ROLE;
UPDATE public.dp_colaboradores SET valor_diaria=500 WHERE id=current_setting('l02.emp')::uuid;
...
DO $verify$ DECLARE v record; BEGIN
 ...
 IF (v.remuneracao_snapshot->>'valor_unitario')::numeric IS DISTINCT FROM 200 OR (v.remuneracao_snapshot->>'quantidade_prevista')::numeric IS DISTINCT FROM 1 OR (v.remuneracao_snapshot->>'valor_previsto')::numeric IS DISTINCT FROM 200 THEN RAISE EXCEPTION 'partial remuneration incorrect: %',v.remuneracao_snapshot; END IF;
...
END $verify$;
SELECT 'partial approval passed; daily rate stays 200 for one day despite current rate 500; timestamps updated; worker/foreign approval denied; repeat rejected without mutation' result;
ROLLBACK;
