...
END $identity$;
SET LOCAL ROLE authenticated;
DO $first$ DECLARE v jsonb; BEGIN v:=public.dp_convocacao_decidir_parcial(current_setting('l02.offer')::uuid,'APROVAR'); IF (v->>'ok')::boolean IS DISTINCT FROM true THEN RAISE EXCEPTION 'first action failed: %',v; END IF; END $first$;
RESET ROLE;
DO $identity$ BEGIN
...
END $identity$;
SET LOCAL ROLE authenticated;
DO $second$ DECLARE v jsonb; BEGIN v:=public.dp_convocacao_responder_oferta(current_setting('l02.reoffer')::uuid,true,NULL::text,NULL::time,NULL::time,NULL::boolean,NULL::text,NULL::text); IF v->>'motivo' IS DISTINCT FROM 'INVALID_STATE' THEN RAISE EXCEPTION 'losing action not denied: %',v; END IF; END $second$;
RESET ROLE;
DO $check$ BEGIN
...
END $check$;
SELECT 'approve first: competing action refused; one accepted offer' result;
ROLLBACK;
