...
SET LOCAL ROLE authenticated;
RESET ROLE;
UPDATE public.dp_convocacoes SET remuneracao_snapshot=NULL WHERE id=current_setting('l02.offer')::uuid;
SET LOCAL ROLE authenticated;
DO $invalid$ BEGIN
 BEGIN
  PERFORM public.dp_convocacao_decidir_parcial(current_setting('l02.offer')::uuid,'APROVAR');
  RAISE EXCEPTION 'missing snapshot accepted';
 EXCEPTION WHEN invalid_parameter_value THEN
  IF SQLERRM NOT LIKE 'INVALID_REMUNERATION_SNAPSHOT:%' THEN RAISE; END IF;
 END;
END $invalid$;
RESET ROLE;
DO $verify$ BEGIN
 IF NOT EXISTS(SELECT 1 FROM public.dp_convocacoes WHERE id=current_setting('l02.offer')::uuid AND status='pendente' AND parcial_status='aguardando_gestor') THEN RAISE EXCEPTION 'invalid snapshot changed offer'; END IF;
END $verify$;
SELECT 'missing historical snapshot rejected; pending proposal preserved' result;
ROLLBACK;
