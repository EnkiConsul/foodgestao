...
const quebrados = rows.filter((r) => Number(r.runs_failed) > 0);
const nuncaRodou = rows.filter((r) => r.active && !r.last_run);
const inativos = rows.filter((r) => !r.active);
...
console.log(`cron-healthcheck: ${rows.length} jobs | falhas 24h: ${quebrados.length} | nunca executados: ${nuncaRodou.length} | inativos: ${inativos.length}`);
...
if (REQUIRE && (quebrados.length > 0 || nuncaRodou.length > 0)) process.exit(1);
