  ...
  try {
    ...
    try {
      if (payment?.id) {
        ...
        // Auto-create a row for recurring invoices generated by Asaas
        if (!inv && payment.subscription) {
          const { data: subRow } = await admin
            ...
            .maybeSingle();
          if (subRow) {
            const billingType = (payment.billingType ?? "").toUpperCase();
            const method =
              billingType === "PIX" ? "pix" :
              billingType === "BOLETO" ? "boleto" :
              billingType === "CREDIT_CARD" ? "credit_card" : null;
            const { data: inserted } = await admin.from("invoices").insert({
        ...
        if (inv) {
          if (["PAYMENT_CONFIRMED", "PAYMENT_RECEIVED", "PAYMENT_RECEIVED_IN_CASH"].includes(eventType)) {
            ...
            await admin.from("invoices").update({
              status: "paid",
              paid_at: nowIso,
              amount_cents: Math.round(Number(payment.value ?? inv.amount_cents / 100) * 100),
            }).eq("id", inv.id);
            ...
            // Value/due date may have changed (e.g., extras added)
            await admin.from("invoices").update({
              amount_cents: Math.round(Number(payment.value ?? inv.amount_cents / 100) * 100),
              due_date: payment.dueDate ?? inv.due_date,
              external_payment_url: payment.invoiceUrl ?? inv.external_payment_url,
            }).eq("id", inv.id);
          }
        }
      }
