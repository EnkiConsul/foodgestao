  ...
  try {
    ...
    if (acao === "listar") {
      ...
      const { data, error } = await admin
        ...
        .select(
          "id, company_id, candidato_nome, whatsapp, cpf, status, cargo_previsto_id, unidade_prevista_id, regime_previsto, " +
    ...
    const montar = async () => {
      ...
      // Aviso (não bloqueio): o CPF informado já existe na empresa? Na
      // recontratação, dados contratuais antigos nunca voltam para a ficha: o
      // novo vínculo deve usar cargo, remuneração e jornada atuais.
      let cpfExistente: {
