...
BEGIN
    ...
    FOR pessoa IN SELECT * FROM jsonb_array_elements(p_pessoas) LOOP
      ...
      ELSE
        INSERT INTO public.dp_preadmissao_pessoas
          (preadmissao_id, company_id, nome, parentesco, cpf, data_nascimento,
           finalidade_dependente, finalidade_sesc)
        VALUES (pa.id, pa.company_id, upper(v_nome), upper(v_par), v_cpf, v_nasc::date, v_dep, v_sesc)
  ...
  UPDATE public.dp_preadmissoes
     SET dados = COALESCE(p_dados, dados),
         cpf = COALESCE(NULLIF(p_campos->>'cpf', ''), cpf),
         email = COALESCE(NULLIF(p_campos->>'email', ''), email),
         data_nascimento = COALESCE(NULLIF(p_campos->>'data_nascimento', '')::date, data_nascimento),
         estado_civil = COALESCE(NULLIF(p_campos->>'estado_civil', ''), estado_civil),
