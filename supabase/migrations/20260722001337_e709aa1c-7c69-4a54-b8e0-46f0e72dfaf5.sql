...
-- Avaliador em runtime das regras de bloqueio (dp_bloqueio_regras)
CREATE OR REPLACE FUNCTION public.dp_regra_bloqueia_data(
  _company_id uuid,
  _unidade_id uuid,
  _data date
) RETURNS boolean
...
BEGIN
  ...
  LOOP
    ...
    ELSIF r.tipo = 'dinamica' AND v_tipo_orig = 'pos_pagamento' THEN
      ...
      -- primeiro sábado após v_pos_dia no mês da data
...
REVOKE ALL ON FUNCTION public.dp_regra_bloqueia_data(uuid, uuid, date) FROM public;
GRANT EXECUTE ON FUNCTION public.dp_regra_bloqueia_data(uuid, uuid, date) TO authenticated, service_role;

-- Trigger de marcação direta: soma regras dinâmicas ao bloqueio manual
...
BEGIN
  ...
  IF public.dp_regra_bloqueia_data(NEW.company_id, v_unidade, NEW.data) THEN
    -- respeita liberação individual: se existir registro em dp_datas_bloqueadas
    -- para essa data com liberada_por_solicitacao preenchido, permite.
    SELECT (liberada_por_solicitacao IS NOT NULL) INTO v_liberada
      FROM public.dp_datas_bloqueadas
     WHERE company_id = NEW.company_id
       AND data = NEW.data
       AND (unidade_id IS NULL OR unidade_id = v_unidade)
...
-- Trigger de solicitações: também aplica a avaliação em runtime
