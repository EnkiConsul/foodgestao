...
BEGIN
  ...
  WITH empresas AS (
    ...
    SELECT cp.*, c.id AS colaborador_id, c.nome AS colaborador_nome,
           ...
           -- Afastamento aprovado cobrindo TODOS os dias trabalháveis do mês
           -- (interseção do mês com admissão/desligamento): não há ponto a bater.
           EXISTS (
...
DROP TRIGGER IF EXISTS trg_dp_solicitacoes_pending_dirty ON public.dp_solicitacoes;
CREATE TRIGGER trg_dp_solicitacoes_pending_dirty
AFTER INSERT OR UPDATE OR DELETE ON public.dp_solicitacoes
