...
-- 1. Extend enum
ALTER TYPE public.transaction_type ADD VALUE IF NOT EXISTS 'parcelado';

-- 2. Direction enum
DO $$ BEGIN
  CREATE TYPE public.parcel_direction AS ENUM ('entrada', 'saida');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- 3. Column
ALTER TABLE public.transactions
  ADD COLUMN IF NOT EXISTS parcel_direction public.parcel_direction;
...
-- 5. Validation trigger for installments
...
AS $$
BEGIN
  IF NEW.transaction_type = 'parcelado'::public.transaction_type THEN
    IF NEW.parcel_direction IS NULL THEN
      RAISE EXCEPTION 'parcel_direction é obrigatório para lançamento parcelado' USING ERRCODE = '22023';
    END IF;
    IF NEW.installment_total IS NULL OR NEW.installment_total < 2 THEN
      RAISE EXCEPTION 'installment_total deve ser >= 2 para parcelado' USING ERRCODE = '22023';
    END IF;
...
DROP TRIGGER IF EXISTS trg_validate_installment_transaction ON public.transactions;
CREATE TRIGGER trg_validate_installment_transaction
BEFORE INSERT OR UPDATE ON public.transactions
FOR EACH ROW EXECUTE FUNCTION public.validate_installment_transaction();

-- 6. Update balance functions to consider 'parcelado'
...
BEGIN
  ...
  SELECT COALESCE(SUM(
    CASE
      ...
      WHEN t.transaction_type::text = 'parcelado' AND t.account_id = _account_id AND t.installment_number IS NOT NULL AND t.parcel_direction = 'entrada' THEN t.amount
      WHEN t.transaction_type::text = 'parcelado' AND t.account_id = _account_id AND t.installment_number IS NOT NULL AND t.parcel_direction = 'saida' THEN -t.amount
      ELSE 0
    END
  ), 0)
  INTO _movements
...
AS $function$
  SELECT COALESCE(SUM(
    CASE
      ...
      WHEN transaction_type::text = 'parcelado' AND installment_number IS NOT NULL AND parcel_direction = 'entrada' THEN amount
      WHEN transaction_type::text = 'parcelado' AND installment_number IS NOT NULL AND parcel_direction = 'saida' THEN -amount
      ELSE 0
    END
  ), 0)::numeric
  ...
  RETURN QUERY
    SELECT to_char(date_trunc('month', t.transaction_date), 'YYYY-MM') AS mes,
           COALESCE(SUM(CASE
             ...
             WHEN t.transaction_type::text='parcelado' AND t.installment_number IS NOT NULL AND t.parcel_direction='entrada' THEN t.amount
           END),0)::numeric,
           COALESCE(SUM(CASE
             WHEN t.transaction_type='despesa' THEN t.amount
             WHEN t.transaction_type::text='parcelado' AND t.installment_number IS NOT NULL AND t.parcel_direction='saida' THEN t.amount
           END),0)::numeric,
           COALESCE(SUM(CASE
             WHEN t.transaction_type='receita' THEN t.amount
             WHEN t.transaction_type='despesa' THEN -t.amount
