-- 1) Setor da pessoa avulsa (folguista / em teste / registro manual do dia)
...
CREATE INDEX IF NOT EXISTS idx_dp_pessoas_avulsas_setor
  ON public.dp_pessoas_avulsas (setor_id) WHERE setor_id IS NOT NULL;

CREATE OR REPLACE FUNCTION public.dp_pessoa_avulsa_validar_setor()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
DROP TRIGGER IF EXISTS trg_dp_pessoa_avulsa_validar_setor ON public.dp_pessoas_avulsas;
CREATE TRIGGER trg_dp_pessoa_avulsa_validar_setor
  BEFORE INSERT OR UPDATE OF setor_id, unidade_id, company_id ON public.dp_pessoas_avulsas
  FOR EACH ROW EXECUTE FUNCTION public.dp_pessoa_avulsa_validar_setor();

-- 2) Setor do dia da pessoa avulsa via RPC (nunca recebe id sintético de tela)
...
BEGIN
  ...
  IF NOT private.is_company_admin_or_owner(v_reg.company_id) THEN
...
-- 3) Marcar / desmarcar o dia como resolvido (idempotente, com e sem unidade)
...
BEGIN
  ...
  END IF;
  IF NOT private.is_company_admin_or_owner(p_company) THEN
