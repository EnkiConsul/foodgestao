-- Modo de combinação das regras de adicional por tempo de serviço:
-- 'escada' (maior ciclo alcançado vence) ou 'cumulativo' (soma todas).
ALTER TABLE public.dp_config_dp
  ADD COLUMN IF NOT EXISTS adicional_tempo_servico_modo text NOT NULL DEFAULT 'escada';

ALTER TABLE public.dp_config_dp
  DROP CONSTRAINT IF EXISTS dp_config_dp_adicional_modo_chk;
ALTER TABLE public.dp_config_dp
  ADD CONSTRAINT dp_config_dp_adicional_modo_chk
  CHECK (adicional_tempo_servico_modo IN ('escada','cumulativo'));
...
BEGIN
  ...
  SELECT salario_familia_cota, salario_familia_teto, salario_familia_vigencia,
         ...
         COALESCE(adicional_tempo_servico_modo, 'escada')
    INTO v_sf_cota, v_sf_teto, v_sf_vig, v_adic_ativo, v_modo
  ...
  LOOP
    ...
    IF v_periodo.tipo = 'contracheque_mensal' THEN
      ...
      -- Modo escada: vence o maior ciclo já alcançado. Cumulativo: soma todas.
      v_adic := 0;
      ...
      ELSIF v_adic_ativo AND r.data_admissao IS NOT NULL THEN
        ...
        FOR v_regra IN
          SELECT a.*
           ...
           ORDER BY CASE a.escopo WHEN 'cargo' THEN 4 WHEN 'unidade' THEN 3 WHEN 'sindicato' THEN 2 ELSE 1 END DESC,
                    CASE WHEN floor(v_meses / a.ciclo_meses) >= 1 THEN 1 ELSE 0 END DESC,
                    a.ciclo_meses DESC,
        ...
        LOOP
          ...
          IF v_perc > 0 THEN
            ...
            END IF;
          END IF;
          IF COALESCE(v_modo, 'escada') <> 'cumulativo' THEN EXIT; END IF;
        END LOOP;
      END IF;
