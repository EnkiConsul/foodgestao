-- Resumo de valores do dia de convocação (versão 2):
-- prêmio percentual sem salário base usa a remuneração do próprio dia e o
-- resumo passa a declarar o divisor do DSR e sua versão.
...
BEGIN
  ...
  -- Prêmio de assiduidade rateado por dia trabalhado. Em percentual, a base é o
  -- salário base quando existe; quem é pago por hora ou diária não tem salário
  -- base, então o percentual incide sobre a remuneração do próprio dia.
  IF COALESCE(v_c.premio_assiduidade, false) AND COALESCE(v_c.premio_assiduidade_valor, 0) > 0 THEN
    IF COALESCE(v_c.premio_assiduidade_tipo, 'valor') = 'percentual' THEN
      v_premio_base := LEAST(GREATEST(v_c.premio_assiduidade_valor, 0), 100) / 100;
      v_premio_dia := CASE
        WHEN COALESCE(v_c.salario_base, 0) > 0 THEN v_c.salario_base * v_premio_base / 22
        ELSE (v_valor * v_qtd) * v_premio_base
      END;
    ELSE
      v_premio_dia := v_c.premio_assiduidade_valor / 22;
    END IF;
  END IF;
...
-- Recalcula, somente para leitura, o resumo de convocações que ainda podem ser
-- pagas (pendente/aceita). Convocações encerradas conservam o resumo histórico.
...
BEGIN
  ...
  SELECT c.id,
   ...
   WHERE c.id = ANY(p_ids)
     ...
     AND (private.dp_regras_membro(c.company_id) OR c.colaborador_id = v_meu);
END;
$function$;

REVOKE ALL ON FUNCTION public.dp_convocacoes_remuneracao_atual(uuid[]) FROM PUBLIC;
