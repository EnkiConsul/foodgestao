CREATE OR REPLACE FUNCTION public.link_open_finance_account(
  _of_account_id uuid,
  _local_account_id uuid DEFAULT NULL,
  _local_credit_card_id uuid DEFAULT NULL,
  _auto_import boolean DEFAULT NULL
)
RETURNS public.open_finance_accounts
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.companies WHERE id = _company_id AND owner_id = _uid) THEN
    _is_admin := true;
  ELSE
...
REVOKE ALL ON FUNCTION public.link_open_finance_account(uuid, uuid, uuid, boolean) FROM public;
GRANT EXECUTE ON FUNCTION public.link_open_finance_account(uuid, uuid, uuid, boolean) TO authenticated;