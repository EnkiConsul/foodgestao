...
BEGIN
  ...
  IF v_links > 0 OR v_sessoes_antes IS NULL OR v_sessoes_antes < v_agora - interval '5 seconds' THEN
    INSERT INTO public.audit_logs (user_id, action, entity_type, entity_id, company_id, details, actor_kind)
    VALUES (_actor, 'portal_access_revoked', 'dp_portal_acesso', v_colab.user_id::text,
            v_colab.company_id,
            jsonb_build_object(
