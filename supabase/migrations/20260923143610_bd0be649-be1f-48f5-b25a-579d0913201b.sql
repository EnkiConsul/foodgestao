...
BEGIN
  ...
  WHERE u.deleted_at IS NULL
    ...
    -- contas internas do portal do colaborador não acessam o Backoffice
    AND u.email NOT ILIKE '%@portal.360food.local'
    AND NOT EXISTS (
      SELECT 1 FROM public.user_roles r
      WHERE r.user_id = u.id AND r.role IN ('dp_colaborador', 'super_admin')
    )
    AND (
      termo IS NULL
...
BEGIN
  ...
  IF alvo_email ILIKE '%@portal.360food.local'
     OR EXISTS (
       SELECT 1 FROM public.user_roles r
