...
BEGIN
  ...
  -- Cria/atualiza lançamento de pagamento (despesa na conta bancária)
  ...
  UPDATE public.credit_card_invoices
     SET paid_amount            = v_new_paid,
         ...
         status                 = v_new_status::text::public.credit_card_invoices_status_enum_or_text,
         paid_at                = CASE WHEN v_new_status = 'paga' THEN now() ELSE paid_at END,
         updated_at             = now()
   WHERE id = v_inv.id;

  -- Rotativo: rola remanescente + juros para próxima fatura
  IF v_remainder > 0 THEN
    -- Localiza/cria próxima fatura
    SELECT * INTO v_next
    ...
    IF v_interest > 0 THEN
      -- categoria "Despesas Financeiras" (fallback: qualquer categoria de despesa do usuário/empresa)
      ...
      -- lançamento informativo dentro da próxima fatura (sem afetar conta bancária: usa a conta do cartão)
      INSERT INTO public.transactions(
