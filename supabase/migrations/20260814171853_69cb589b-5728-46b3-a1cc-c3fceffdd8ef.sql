-- 1) Normaliza intermitentes cadastrados como mensalistas (forma incompatível com o contrato)
UPDATE public.dp_colaboradores
   SET valor_hora = COALESCE(valor_hora, round(salario_base / 220.0, 2)),
       forma_pagamento = 'horista',
       salario_base = NULL
 WHERE regime = 'intermitente'
   AND forma_pagamento = 'mensalista'
   AND COALESCE(salario_base, 0) > 0;

-- 2) Pendências de remuneração ignoram freelancers (não entram em folha CLT)
CREATE OR REPLACE FUNCTION public.dp_folha_pendencias_remuneracao(_company_id uuid)
 RETURNS TABLE(colaborador_id uuid, nome text, forma_pagamento dp_forma_pagamento, motivo text)
 LANGUAGE sql
...
-- 3) Geração da folha ignora freelancers
