  ...
  const salvarAvulsa = useMutation({
    mutationFn: async (input: PessoaAvulsaInput) => {
      ...
      const { error } = await supabase.rpc("dp_pessoa_avulsa_salvar", {
        ...
        p_tipo: input.tipo,
        p_nome: manual ? undefined : input.nome?.trim() ?? undefined,
        p_colaborador: (manual ? input.colaborador_id ?? null : null) ?? undefined,
        p_cobre: (cobre) ?? undefined,
        p_cobre_motivo: (cobre ? input.cobre_motivo ?? null : null) ?? undefined,
        p_data_inicio: input.data_inicio,
        p_data_fim: input.data_fim,
        p_entrada: (input.entrada || null) ?? undefined,
        p_saida: (input.saida || null) ?? undefined,
        p_termina: !!input.termina_no_dia_seguinte,
        p_observacao: (input.observacao?.trim() || null) ?? undefined,
        p_telefone: (manual ? null : input.telefone?.trim() || null) ?? undefined,
        p_apoio: manual ? undefined : input.pessoa_apoio_id ?? undefined,
