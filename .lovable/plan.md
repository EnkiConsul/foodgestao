...
- `src/hooks/useEdgeGestures.ts`: manter borda/limiares atuais; no gesto esquerda→direita, quando `pathname === homeTo` e o módulo ativo for `dp`, navegar para `/hub` ou `/dp/analytics` conforme a contagem de módulos ativos (`useCompanyModules`); o gesto direita→esquerda continua indo para `moreTo` em todos os módulos, sem encadeamento extra no portal.
