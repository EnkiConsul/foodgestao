...
- Portal: `DpMeuHome.tsx` com `toProperName` no primeiro nome; `useDpDocumentos.tsx` ordenando por competência/`data_referencia`; `DpMeuEscala.tsx` listando apenas dias com trabalho confirmado (convocações aceitas + itens de escala do próprio colaborador), sem varrer `diasDaCompetencia`; `mobileNav`/`.dp-shell` com `env(safe-area-inset-bottom)`; atalho de disponibilidade em `dpNavigation.tsx`.
