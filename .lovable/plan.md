...
- Exclusão de rascunho: nova RPC `dp_convocacao_excluir_grupo` (só em `status = 'rascunho'`, autorizada a gestores; marca grupo e ocorrências como canceladas, preservando histórico — sem remoção física) + botão no `GrupoCard` com diálogo de confirmação e invalidação das consultas.
