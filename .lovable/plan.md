...
- Minha Escala: mostra só os dias que importam para a pessoa — os dias aceitos (marcados como confirmados) e, junto, os dias convocados que ainda estão no prazo de resposta (marcados como "aguardando sua resposta", com atalho para responder). Nada de listar o mês inteiro. A frase "a escala deste mês ainda não foi publicada" sai; sem nenhum dia, aparece "Você ainda não tem dia confirmado neste mês".
...
- Intermitente: o valor mostrado passa a ser um resumo com as parcelas — horas × valor-hora, adicional noturno das horas em faixa noturna, 1/12 de 13º proporcional, 1/12 de férias proporcional com o terço, prêmio de assiduidade quando cadastrado e o vale-alimentação do dia. Abaixo, os descontos e encargos do dia: INSS sobre a remuneração do dia e FGTS (que é custo da empresa, mostrado à parte, sem descontar). O bloco fecha com "total a receber no dia" (remuneração − INSS) e a marca de que é estimativa.
...
- Portal: `DpMeuHome.tsx` com `toProperName` no primeiro nome; `useDpDocumentos.tsx` ordenando por competência/`data_referencia`; `DpMeuEscala.tsx` listando apenas dias relevantes — itens de escala e convocações aceitas do colaborador mais convocações `pendente` com `prazo_resposta`/início ainda futuros (via `dp_convocacao_minhas_ofertas`), sem varrer `diasDaCompetencia`; `mobileNav`/`.dp-shell` com `env(safe-area-inset-bottom)`; atalho de disponibilidade em `dpNavigation.tsx`.
- Convocação: nova função pura `src/lib/dp/convocacao-remuneracao.ts` (parcelas do dia, noturno via faixas do turno, 13º/férias 1/12, VA de `useDpValeCalculadora`, INSS pela tabela progressiva de `lib/dp/encargos.ts` e FGTS 8% informativo, líquido do dia), com testes puros; usada no snapshot da convocação e nas telas do gestor e do portal; migração em `dp_convocacoes` com `freela_valor_diaria`, `freela_refeicao_tipo`, `freela_refeicao_valor`, `freela_transporte_valor`, `freela_gorjeta`, `observacoes_convite` e campos exibidos condicionalmente em `NovaConvocacaoPlanner.tsx`/`RevisaoConvocacao.tsx`/`DpMinhasConvocacoes.tsx`.
