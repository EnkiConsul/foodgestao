...
2. `DROP CONSTRAINT` + `ADD CONSTRAINT` das duas FKs de substituição como compostas contra `dp_convocacoes(id, company_id)`, com `ON DELETE SET NULL (substituida_por_id)` / `ON DELETE SET NULL (substitui_convocacao_id)` — a limpeza atinge **somente a coluna do ponteiro**; `company_id` permanece intacto e NOT NULL. Confirmo antes a versão do Postgres (SET NULL por coluna exige PG 15+); se indisponível, uso alternativa comprovada (`ON DELETE RESTRICT` + trigger de limpeza do ponteiro) e justifico. Sem CASCADE.
3. Descumprimentos: inventariar os CHECKs atuais e substituir **apenas** os redundantes de percentual/regime/análise (`dp_conv_descump_percentual_faixa_check`, `dp_conv_descump_percentual_regime_check`, `dp_conv_descump_percentual_analise_check`) por um CHECK consolidado: `percentual_referencia IS NULL OR (percentual_referencia = 50 AND regime_snapshot = 'intermitente' AND analise = 'sem_justo_motivo' AND parte_responsavel IN ('colaborador','empregador'))`. Preservar intactas as constraints independentes de base e valor (`base_positiva`, `valor_analise`, `valor_base`, `valor_positivo`), o CHECK `tipo × parte_responsavel` e o de coerência da análise.
4. `REVOKE ALL ... FROM anon, authenticated` nas 6 tabelas novas; nenhum privilégio a `anon`. Reconceder `SELECT` a `authenticated` em `dp_convocacao_grupos`, `dp_convocacao_ocorrencias`, `dp_convocacao_config`, `dp_indisponibilidades`, `dp_convocacao_descumprimentos` **e também em `dp_convocacao_eventos`** — leitura de auditoria preservada, limitada pela policy RLS de admin/owner; append-only = sem INSERT/UPDATE/DELETE direto, não ausência de leitura. `service_role` mantém `ALL`.
