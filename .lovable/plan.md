...
- Fechar diálogo por swipe: no `Tabs`/gesto lateral já existente (`src/lib/nav/tabSwipe.ts`), quando o toque ocorre dentro de um `[role="dialog"][data-state="open"]` e o arrasto é esquerda→direita, se não houver aba anterior (ou o diálogo não tiver abas) o gesto dispara o fechamento do diálogo — via clique no botão de fechar do `DialogContent` ou tecla `Escape`, para que os diálogos que já interceptam fechamento (confirmação de alterações não salvas em `ColaboradorFormDialog` e similares) continuem mostrando a pergunta de salvar/descartar. Diálogos sem controle de alterações fecham direto.
- Testes: unitários da função de próximo/anterior menu (pontas, Início como origem, retorno ao Início) e da decisão "trocar aba x fechar diálogo"; validação por Playwright em viewport mobile simulando arrastos verticais no Início/Cadastro e o arrasto de fechar em um diálogo com e sem alterações.
