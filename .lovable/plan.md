...
2. **Ícones condicionais**: a seta de ordenação aparece **apenas quando a coluna é a ordenação ativa** (`sortAtivo`); quando inativa, não mostrar seta solta. O ícone de filtro aparece sempre, destacado (`text-primary`) quando há filtros ativos e sutil (`opacity-40`) quando não há.
