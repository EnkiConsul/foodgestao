...
- Migração: tabela `dp_adiantamento_solicitacoes` (`company_id`, `colaborador_id`, `tipo` ativar/cancelar, `data_solicitacao`, `competencia_efeito`, `origem` gestor/portal, `observacao`, `criado_por`, timestamps) com GRANTs, RLS por empresa, política de inserção do próprio colaborador e trigger que bloqueia data retroativa ou a menos de 5 dias do pagamento quando `origem = 'portal'`. `optante_adiantamento` deixa de ser editado na UI (mantido apenas como cache derivado, atualizado por trigger a partir da última solicitação) e o valor atual é migrado como solicitação inicial. Helper novo `src/lib/dp/adiantamento-opcao.ts` com `competenciaEfeito(dataSolicitacao, diaPagamento)` e `ativoNaCompetencia(solicitacoes, competencia)` (última solicitação com efeito ≤ competência); hook `useDpAdiantamentoSolicitacoes.tsx`; UI de solicitações em `ColaboradorFormDialog.tsx`/ficha (com data retroativa permitida) e no portal (`src/pages/dp/portal/`, mínimo hoje + antecedência de 5 dias), notificando o gestor via `dp_notificacoes`.
