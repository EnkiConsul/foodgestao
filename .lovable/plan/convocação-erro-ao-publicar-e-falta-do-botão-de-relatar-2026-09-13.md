...
- Novo helper `notifyError` (ex.: `src/lib/notifyError.ts`): `toast.error` com texto amigável + `reportError` no mesmo chamado; substitui os ~40 pontos que hoje fazem `toast.error(e?.message)` nas telas DP/financeiras principais (lotes de arquivos por rodada, mantendo mensagens já amigáveis intactas).
